...
- (NSArray *)landmarks:(cv::Mat)captured_image frame__:(int)frame_count fx__:(double)fx fy__:(double)fy cx__:(double)cx cy__:(double)cy {
    // Reading the images
    cv::Mat_<float> depth_image;
    cv::Mat_<uchar> grayscale_image;
    ...
    std::vector<cv::Point2d> landmarks = LandmarkDetector::CalculateLandmarks(clnf_model.detected_landmarks, clnf_model.patch_experts.visibilities[0][idx]);
    return [self convertFrom:landmarks];
}
...
- (NSArray *)convertFrom:(std::vector<cv::Point2d>)landmarks {
    NSMutableArray *results = [NSMutableArray array];
    int i = 0;
    for (std::vector<cv::Point2d>::iterator it = landmarks.begin(); it != landmarks.end(); it++, i++) {
        cv::Point2d cvPoint = landmarks[i];
        CGPoint point = CGPointMake(cvPoint.x, cvPoint.y);
        [results addObject:[NSValue valueWithCGPoint:point]];
    }
    return results;
}
@end
